...
void main()
{
    ...
    else // If it's another vanilla armor, for example diamond_layer_1.png or diamond_layer_2.png
    {
        if (IS_CUSTOM_EMISSIVE) {
            ...
            (1, 1): textureProperites (e, s, #, 1)
            if e == 1:
                second half (horizontally) of texture becomes emissive map
            if e > 1:
                everything is AWESOME err.. emissive

            if s > 1:
                halve the texture (ignored emissive map)
            */
            vec4 textureProperties = texelFetch(Sampler0, ivec2(1, 1), 0);
            textureProperties.rgb *= textureProperties.a * 255;
            vec4 vtc = vertexColor;
            vec2 localCoords = texCoord0;

            float lh_offset = 0.5;

            bool force_half = textureProperties.g > 1;

            if (textureProperties.r == 1) {
                localCoords.x /= 2;
                if (texture(Sampler0, vec2(localCoords.x + lh_offset, localCoords.y)).a != 0)
                {
                    vtc = vec4(1) * texture(Sampler0, vec2(localCoords.x+lh_offset, localCoords.y)).a;
                }
            } else if (textureProperties.r > 1) {
                if (force_half) {
                    localCoords.x /= 2;
            ...
            color = texture(Sampler0, localCoords) * vtc * ColorModulator;
        } else {
